...
export class TrainingTask {
  ...
  _reset() {
    ...
    this.impactSpeed = 0;    // 着床時の降下速度の最大値 [m/s]
    this._prevAttached = false;
  ...
  start(sim) {
    ...
    this.elTime.textContent = '0.0 s';
    this.elMaxSway.textContent = '0.00 m';
  ...
  update(rs, dt) {
    ...
    if (this.state === 'await-lift') {
      ...
      // 据え付け完了 = 目標ゾーン内で接地し玉掛け解除(玉外しの瞬間のみ判定)
      if (this._prevAttached && !rs.loadAttached && rs.loadOnGround) {
      ...
      this._prevAttached = rs.loadAttached;
      this._prevOnGround = rs.loadOnGround;
  ...
  _finish(err) {
    this.state = 'done';
    // 採点: 基準時間 60s、振れ・据付誤差・着床衝撃・斜め吊りで減点
    const tPen = Math.max(0, (this.time - 60) * 0.5);
    const swayPen = this.maxSway * 25;
    const errPen = err * 20;
    const impactPen = Math.max(0, this.impactSpeed - 0.05) * 100;
    const obliquePen = this.obliqueTime * 2;
    const score = Math.max(0, Math.round(100 - tPen - swayPen - errPen - impactPen - obliquePen));
    ...
    this.elResult.innerHTML =
      ...
      `着床衝撃: ${this.impactSpeed.toFixed(2)} m/s(減点 ${impactPen.toFixed(1)})<br>` +
